...
def criar_item(tarefas, tarefa, status, frame_tarefas):
  ...
  check = tk.Checkbutton(frame_item, text=tarefa, variable=val_checkbox, bg="#001F3F", command=lambda: alterar_valor(tarefas, tarefa, val_checkbox, check), font=("Arial", 10, "bold"))
  ...
  botao_excluir = tk.Button(frame_item, text="Excluir", bg="lightcoral", font=("Arial", 10, "bold"),command=lambda: excluir_tarefa(tarefas, tarefa, frame_tarefas))
  botao_excluir.pack(side="right", padx=5)

  botao_editar = tk.Button(frame_item, text="Editar", bg="#0D6EFD", fg="#FFFFFF", font=("Arial", 10, "bold"),command=lambda: editar_tarefa(tarefas, tarefa, frame_tarefas))
...
def editar_tarefa(tarefas, tarefa, frame_tarefas):
  ...
  label_editar = tk.Label(frame_editar, text="Nova descrição", bg="#001F3F", fg="#FFF", font=("Arial", 10, "bold"))
  label_editar.pack()
  entrada_editar = tk.Entry(frame_editar, width=20, bg="#1F4E78", fg="#FFF")
  entrada_editar.insert(0, tarefa)
  entrada_editar.pack()

  botao_salvar = tk.Button(frame_editar, text="Salvar", bg="lightgreen",command=lambda: salvar_edicao(tarefas, tarefa, entrada_editar, nova_janela, frame_tarefas))
  botao_salvar.pack(padx=7, pady=7)

  botao_cancelar = tk.Button(frame_editar, text="Cancelar", bg="lightcoral", fg="#FFFFFF", command=nova_janela.destroy)
